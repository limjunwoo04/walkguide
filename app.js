...
async function start() {
  ...
  try {
    ...
    session = await ort.InferenceSession.create("model.onnx?v=6", { executionProviders: ["webgpu"] });
    backend = "webgpu";
  } catch (e) {
    console.warn("WebGPU 실패 → WASM 폴백:", e.message);
    session = await ort.InferenceSession.create("model.onnx?v=6", { executionProviders: ["wasm"] });
    backend = "wasm";
  }
